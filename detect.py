...
import datetime
import json
b_json_open = open('basepoint_config.json', 'r')
b_json_load = json.load(b_json_open)

m_json_open = open('map_config.json', 'r')
m_json_load = json.load(m_json_open)
#------
...
def checkRect(center_point):
    ...
    for feild in range(len(m_json_load)):
        ...
        polygon = Polygon([(polygon_point_list[0],polygon_point_list[1]),
            (polygon_point_list[2],polygon_point_list[3]),
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for feild in range(len(m_json_load)):
                    polygon_point_list = []
                    for p in m_json_load[feild-1]['points']:
                        polygon_point_list.append(b_json_load[p-1]['points']['x'])
                        polygon_point_list.append(b_json_load[p-1]['points']['y'])
                    
                    x1 = int(polygon_point_list[0])
                    y1 = int(polygon_point_list[1])
                    x2 = int(polygon_point_list[2])
                    y2 = int(polygon_point_list[3])
                    x3 = int(polygon_point_list[4])
                    y3 = int(polygon_point_list[5])
                    x4 = int(polygon_point_list[6])
                    y4 = int(polygon_point_list[7])
